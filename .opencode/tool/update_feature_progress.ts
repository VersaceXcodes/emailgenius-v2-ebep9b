...
export default tool({
  ...
  execute: async ({ featureId, message }) => {
    const response = await fetch("https://launchpulse.ai/api/project/emailgenius-v2/features/progress", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        projectId: "emailgenius-v2",
        token: "lp_emailgen_330056f5753446ea",
        featureId: featureId,
        passes: true,
